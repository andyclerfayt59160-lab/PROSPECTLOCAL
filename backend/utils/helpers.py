...
def is_directory_listing_url(url: Optional[str]) -> bool:
    """
    Retourne True si l'URL pointe vers un annuaire/profil et non vers le site propre du pro.
    """
    if not url:
        return False
...
def is_hauts_de_france_postal_code(postal_code: Optional[str]) -> bool:
    """
    Détecte si un code postal appartient aux Hauts-de-France.
    """
    if not postal_code:
        return False

    digits = re.sub(r"\D", "", postal_code)
    return len(digits) >= 2 and digits[:2] in HAUTS_DE_FRANCE_PREFIXES


def is_target_france_location(
    postal_code: Optional[str],
    city: Optional[str] = None,
    address: Optional[str] = None,
) -> bool:
    """
    Retourne True si la fiche semble appartenir à la France métropolitaine/DOM
    selon les données de localisation disponibles.

    Règles simples :
    - un code postal à 5 chiffres est accepté
    - un code postal numérique non 5 chiffres est rejeté
    - sans code postal, on rejette seulement si des mots-clés étrangers explicites sont présents
    """
    digits = re.sub(r"\D", "", postal_code or "")
    if digits:
        return bool(FRANCE_POSTAL_CODE_PATTERN.match(digits))

    location_blob = " ".join(part for part in [address, city] if part).lower()
    return not any(keyword in location_blob for keyword in FOREIGN_LOCATION_KEYWORDS)


def resolve_pagesjaunes_state(business_data: dict) -> str:
    """
    Resolve the effective PagesJaunes state for a business.

    Returns one of: "present", "absent", "unknown".
    """
    if not business_data:
        return "unknown"

    if business_data.get("pj_manually_set"):
        return "present" if business_data.get("has_pagesjaunes") else "absent"

    pj_confidence = (business_data.get("pj_confidence") or "").strip().lower()
    if pj_confidence == "confirmed":
        return "present"
    if pj_confidence == "not_found":
        return "absent"

    if business_data.get("has_pagesjaunes") is True:
        return "present"

    return "unknown"


def has_confirmed_pagesjaunes_presence(business_data: dict) -> bool:
    return resolve_pagesjaunes_state(business_data) == "present"


def is_confirmed_pagesjaunes_absent(business_data: dict) -> bool:
    return resolve_pagesjaunes_state(business_data) == "absent"
...
def build_google_place_url(google_place_id: Optional[str]) -> Optional[str]:
    """
    Build a Google Maps verification URL from a place id.
    """
    if not google_place_id:
        return None
    return f"https://www.google.com/maps/search/?api=1&query_place_id={google_place_id}"


def build_phone_data_source(
    phone: Optional[str],
    raw_source: Optional[str] = None,
    phone_confidence: Optional[str] = None,
    phone_source_url: Optional[str] = None,
    website_url: Optional[str] = None,
    pappers_url: Optional[str] = None,
    google_place_id: Optional[str] = None,
    source_date: Optional[str] = None,
) -> Optional[dict]:
    """
    Return a canonical, verifiable phone source descriptor.

    We only keep phone provenance when we can point to a real verification URL.
    """
    if not phone:
        return None
    ...
    normalized_date = source_date or datetime.utcnow().isoformat()

    google_url = phone_source_url or build_google_place_url(google_place_id)
    web_url = phone_source_url or (website_url if is_probable_business_website(website_url) else None)
    pappers_candidate = phone_source_url or pappers_url

    if "pappers" in source_key or "légales" in source_key or "legales" in source_key:
        if not pappers_candidate:
            return None
        return {
            "source": "pappers",
            "source_name": "Pappers (données légales)",
            "url": pappers_candidate,
            "icon": "document-text",
            "confidence": phone_confidence or "haute",
            "date": normalized_date,
        }

    if "google" in source_key or "places" in source_key or "knowledge" in source_key:
        if not google_url:
            return None
        return {
            "source": "google",
            ...
            "url": google_url,
            "icon": "logo-google",
            "confidence": phone_confidence or "moyenne",
            "date": normalized_date,
        }

    if any(token in source_key for token in ["site", "web", "scraping"]):
        if not web_url:
            return None
        return {
            "source": "web",
    ...
    if phone_source_url:
        return {
            "source": "enrichment",
            "source_name": source_label or "Enrichissement web",
            "url": phone_source_url,
            "icon": "sparkles",
            "confidence": phone_confidence or "moyenne",
            "date": normalized_date,
        }
...
def calculate_score(business_data: dict) -> tuple[int, str]:
    ...
    pj_state = resolve_pagesjaunes_state(business_data)
    has_pagesjaunes = pj_state == "present"
    pj_absent = pj_state == "absent"

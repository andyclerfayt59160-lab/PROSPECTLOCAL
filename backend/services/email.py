...
try:
    ...
    logger.warning(
        "Resend library not installed. HTTP fallback will be used when RESEND_API_KEY is configured."
    )
...
def get_email_delivery_status() -> Dict[str, Any]:
    """Describe the current runtime mode for outbound emails."""
    ...
    if resend_api_key and RESEND_AVAILABLE:
        return {
            ...
            "mode": "sdk",
            "label": "Resend SDK",
            "description": "Envoi direct actif via la librairie Resend.",
    ...
    if resend_api_key:
        return {
            ...
            "mode": "http",
            "label": "Resend HTTP",
            "description": "Envoi direct actif via l'API HTTP Resend.",
            "sender_email": sender_email,
        }
    ...
    return {
        ...
        "description": "Aucun fournisseur email configure. Les emails restent en file d'attente.",
        "sender_email": sender_email,
    }


async def _send_email_via_resend_sdk(
    resend_api_key: str,
    sender_email: str,
    to: str,
    subject: str,
    ...
    """Send an email with the official Resend SDK."""
...
async def _send_email_via_resend_http(
    resend_api_key: str,
    sender_email: str,
    to: str,
    subject: str,
    ...
    """Send an email directly to the Resend HTTP API."""
    payload = {
        "from": sender_email,
        "to": [to],
        "subject": subject,
...
async def send_email(
    ...
    resend_api_key = os.environ.get("RESEND_API_KEY")
    sender_email = delivery_status["sender_email"]
    ...
    # If an email provider is configured, try to send immediately.
    if delivery_status["ready"] and resend_api_key:
        try:
            if delivery_status["mode"] == "sdk":
                result = await _send_email_via_resend_sdk(
                ...
                result = await _send_email_via_resend_http(
                    resend_api_key=resend_api_key,
                    sender_email=sender_email,
                    to=to,
                    subject=subject,
            ...
            email_record["provider"] = delivery_status["label"]

            await db.email_queue.insert_one(email_record)

            logger.info(
                f"Email sent successfully to {to}: {subject} [{delivery_status['label']}]"
            )
            return {
                "success": True,
                "status": "sent",

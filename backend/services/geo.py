...
async def geocode_address(
    ...
    """
    Geocode an address using the French BAN API first, then Google Maps fallback.
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if postal_code:
                ban_params["postcode"] = postal_code
            if city:
                ban_params["city"] = city

            ban_response = await client.get("https://api-adresse.data.gouv.fr/search/", params=ban_params)

            if ban_response.status_code == 200:
                ban_data = ban_response.json()
                features = ban_data.get("features", [])
                if features:
                    coordinates = (
                        features[0]
                        .get("geometry", {})
...
async def get_cities_in_radius(
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            # Restrict department candidates to the nearest communes only.
            dept_codes = set()
            ...
            # Fallback: if the reverse lookup was sparse, keep at least the
            # first detected department but avoid widening to hard-coded regions.
            if not dept_codes and nearby_cities:
                first_dept = nearby_cities[0].get("codeDepartement")
                if first_dept:
                    dept_codes.add(first_dept)

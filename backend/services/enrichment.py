...
from utils.helpers import is_probable_business_website
...
async def enrich_with_serper(
    ...
    try:
        ...
        # We only accept a phone when both the business name and the local
        # context (city or postal code) align strongly enough.
        if not result["phone"]:
            for organic_result in serper_data.get("organic", []):
                ...
                # STRICT: require a very strong name signal.
                exact_match = company_name.lower() in title.lower()
                strong_name_match = name_similarity >= 0.85 or exact_match

                # Also check if the local context is mentioned.
                city_in_snippet = city.lower() in snippet.lower() if city else False
                city_in_title = city.lower() in title.lower() if city else False
                postal_in_snippet = postal_code in snippet if postal_code else False
                postal_in_title = postal_code in title if postal_code else False
                local_context_match = city_in_snippet or city_in_title or postal_in_snippet or postal_in_title

                if strong_name_match and local_context_match:
                    ...
                    for pattern in phone_patterns:
                        ...
                        if phone_match:
                            ...
                            if normalized_phone:
                                result["phone"] = normalized_phone
                                confidence = "moyenne"
                                result["phone_source"] = "Recherche Web contextuelle"
                                result["phone_confidence"] = confidence
                                result["enrichment_source"] = "serper_snippet"
                                logger.info(
                                    f"  ✅ Serper Snippet: {company_name} - Phone: {result['phone']} "
                                    f"(similarity: {name_similarity:.2f}, local_context={local_context_match})"
                                )
                                break
                    ...
                    if name_similarity >= 0.3:
                        logger.debug(
                            f"  ⚠️ Skipped Serper phone match: {company_name} vs '{title}' "
                            f"(similarity: {name_similarity:.2f}, local_context={local_context_match})"
                        )
    ...
    async with httpx.AsyncClient(timeout=15.0) as http_client:
        ...
        if not result["phone"] and serper_api_key:
            ...
            if serper_result["phone"]:
                ...
                result["phone_source"] = serper_result.get("phone_source")
                result["phone_confidence"] = serper_result.get("phone_confidence")
            if not result["website"] and is_probable_business_website(serper_result["website"]):
                result["website"] = serper_result["website"]

    if result["website"] and not is_probable_business_website(result["website"]):
        logger.info(f"  ⚠️ Site rejeté car annuaire/légal: {result['website']}")
        result["website"] = ""
